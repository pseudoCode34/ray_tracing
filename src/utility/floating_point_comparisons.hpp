...
/*
 * \brief A tolerance value for floating point roundoff error.
 *
...
inline constexpr T dummy_precision = 1e-4;
 ...
 */
template <std::floating_point T>
[[nodiscard]] constexpr bool near_zero(T number,
									   T max_abs_error = dummy_precision<T>) {
 ...
 */
template <std::floating_point T>
[[nodiscard]] constexpr bool near(T number, T target,
								  T prec = dummy_precision<T>) {
	using std::fabs, std::fmax;
	T diff          = fabs(number - target);
	T max_rel_error = fmax(fabs(number), fabs(target));

	return diff <= prec * max_rel_error;
}

template <std::floating_point T>
[[nodiscard]] constexpr bool nan_sensitive_near(T number, T target,
												T prec = dummy_precision<T>) {
	using std::isnan;
